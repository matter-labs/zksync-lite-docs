...
firstly {
    ...
    wallet.provider.transactionFeePromise(for: .forcedExit,
                                          address: ethSigner.address,
                                          tokenIdentifier: Token.ETH.address).map { ($0, state) }
}.then { (feeDetails, state) -> Promise<String> in
    let fee = TransactionFee(feeToken: Token.ETH.address,
                             fee: feeDetails.totalFeeInteger)
    return wallet.forcedExitPromise(target: state.address,
                                    fee: fee,
                                    nonce: state.committed.nonce)
}.done { hash in
    print("Successfully submitted transaction with hash: \(hash)")
...
firstly {
    ...
    ethereum.fullExit(token: .ETH, accountId: state.id!)
}.done { result in
    print("Successfully performed full exit with result: \(result)")
}.catch { error in
    print("Failed with error: \(error)")
}
```
