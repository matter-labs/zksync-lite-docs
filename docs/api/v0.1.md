...
v0.1 API is considered experimental: while the most interfaces were stabilized, we're still working on the interface
that could be declared "stable". Some of the v0.1 parts were designed a lot of time ago, and may be not really
intuitive.
...
[tx_batches]: ../dev/sending_transactions#sending-transaction-batches
...
```json
$ curl -X POST -H 'Content-type: application/json' -d '{"jsonrpc":"2.0", "id":1, "method": "get_tx_fee", "params": ["Transfer", "0xC8568F373484Cd51FDc1FE3675E46D8C0dc7D246", "DAI"]}' https://rinkeby-api.zksync.io/jsrpc
{
  "jsonrpc": "2.0",
  "result": {
